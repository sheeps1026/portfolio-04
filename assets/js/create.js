...
function createPreview(event) {
  ...
  let button = elem.closest(".note-create");
  if (!button) return;

  const noteCreate = document.querySelectorAll(".note-create");
  const sidebarList = document.querySelectorAll(".sidebar-list");

  const index = Array.prototype.indexOf.call(noteCreate, button);

  if (elem.nodeName === "I" || "P") {
    while (!elem.classList.contains("note-create")) {
  ...
  sidebarList[index].appendChild(sidebarSeparator);
...
document.addEventListener("click", previewBtn);
